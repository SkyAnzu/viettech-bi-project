...
### 1.3. Phạm vi dự án
Trong khuôn khổ đồ án này, hệ thống không hướng tới các tính năng phức tạp thường gặp ở môi trường vận hành thực tế (Production) như xử lý dữ liệu thời gian thực (Real-time), nạp dữ liệu gia tăng (Incremental Load), hay tự động hóa luồng công việc theo lịch trình (Scheduling). Thay vào đó, trọng tâm chính của dự án là xây dựng một pipeline dữ liệu tường minh, nhất quán về mặt logic nghiệp vụ và tối ưu hóa khả năng đối soát giữa các tầng kiến trúc.
...
Với đặc thù dữ liệu tĩnh của dự án, RFM được tính từ các đơn hàng có trạng thái `Completed` và chốt tại một thời điểm snapshot cố định (`2025-01-01`). Việc này giúp đảm bảo tính ổn định của số liệu khi trình bày và đối soát. 

Hệ thống sử dụng phương pháp **phân vị tương đối** để chấm điểm từ `1` đến `5` cho mỗi tiêu chí (5 là tốt nhất, 1 là thấp nhất). Khách hàng được xếp hạng dựa trên sự so sánh trong tập khách hàng có đơn `Completed` tại thời điểm snapshot, thay vì theo một ngưỡng tuyệt đối. Ba điểm số này ghép lại thành mã `rfm_score` (ví dụ: `555`, `412`) và là cơ sở để phân tập khách hàng thành 5 nhóm hành vi quản trị điển hình:
...
Dưới góc độ quản trị, việc ứng dụng mô hình RFM mang lại một giá trị chiến lược quan trọng. Nếu chỉ đánh giá dựa trên doanh thu thuần, nhà quản lý có thể đưa ra những nhận định chưa toàn diện về chất lượng của từng tệp khách hàng. Hệ thống BI tích hợp RFM sẽ giúp giải quyết sâu sắc hơn các bài toán chiến lược như:
...
#### 2.1.2. Khái niệm và lý do dùng mô hình ELT

ETL (Extract - Transform - Load) và ELT (Extract - Load - Transform) là hai mô hình phổ biến trong xây dựng pipeline dữ liệu. Với ETL, dữ liệu được trích xuất từ nguồn, biến đổi ở một tầng xử lý trung gian, sau đó mới được nạp vào kho dữ liệu. Ngược lại, với ELT, dữ liệu thô được nạp trước vào kho dữ liệu hoặc vùng Staging, rồi mới được biến đổi bằng năng lực xử lý của nền tảng lưu trữ/phân tích.

Trong dự án này, mô hình ELT được lựa chọn thay vì ETL truyền thống. Giá trị chính của luồng ELT không nằm ở xử lý thời gian thực, mà ở việc tách rõ các lớp dữ liệu: Raw Snapshot, Staging, Data Mart và Dashboard. Cách tổ chức này giúp từng bước xử lý có thể đối soát, giải thích và kiểm tra minh bạch hơn.

**Lý do chọn ELT trong dự án:**

- **Tận dụng năng lực xử lý của BigQuery:** Dữ liệu thô dạng CSV được nạp trực tiếp lên vùng Staging của Google BigQuery. Các bước biến đổi sau đó được thực thi bằng SQL trên BigQuery, giúp tránh phụ thuộc vào máy local hoặc một máy chủ trung gian để xử lý dữ liệu.
- **Lưu giữ bản sao nguyên bản để đối soát:** Tầng `retailbi_stg` giữ dữ liệu gần với cấu trúc nguồn, giúp kiểm tra số dòng, đối chiếu dữ liệu gốc và truy vết sai lệch khi phát hiện vấn đề ở tầng phân tích.
- **Chuẩn hóa logic tập trung:** Các bước biến đổi quan trọng như tạo Star Schema, xử lý khóa thay thế, chuẩn hóa KPI và tính RFM Snapshot được cố định tại tầng Data Mart. Điều này giúp Power BI sử dụng cùng một nguồn dữ liệu đã chuẩn hóa, giảm rủi ro mỗi Dashboard tự diễn giải chỉ số theo một cách khác nhau.

---

#### 2.1.3. Khái niệm về Kiến trúc Dữ liệu

- **Mô hình Star Schema (Sơ đồ hình sao)**
    - **Định nghĩa:** Là kiến trúc tổ chức dữ liệu trong kho dữ liệu (Data Warehouse), trong đó một bảng sự kiện (Fact Table) đóng vai trò trung tâm và các bảng chiều (Dimension Tables) kết nối trực tiếp xung quanh theo dạng hình sao.
...
Điểm quan trọng ở đây là mỗi lớp dữ liệu có một vai trò riêng: Raw Snapshot để lưu snapshot, Staging để đối soát, Data Mart để phân tích, còn Power BI là lớp diễn giải kết quả cho người dùng cuối. Cách tổ chức nhiều lớp như vậy giúp báo cáo có thể giải thích rõ dữ liệu đã được kiểm soát như thế nào trước khi đi vào Dashboard.
...
| Giai đoạn 3 | Load CSV lên BigQuery Staging bằng Python | Các bảng `stg_*` trong `retailbi_stg` |
...
Trước khi dữ liệu được xuất ra CSV và đưa vào pipeline phân tích, toàn bộ nghiệp vụ bán lẻ của VietTech được mô phỏng trong một hệ thống cơ sở dữ liệu quan hệ (RDBMS) theo mô hình OLTP, với schema `retailbi_oltp`. Đây là lớp nguồn của dữ liệu nghiệp vụ, nơi các giao dịch như tạo đơn hàng, cập nhật sản phẩm, đăng ký khách hàng và ghi nhận hoàn trả phát sinh.

Hệ thống được triển khai trên **MySQL 8.0+ (InnoDB)** với character set `utf8mb4`, chứa dữ liệu trải dài trong phạm vi 2 năm (01/01/2023–31/12/2024).

#### Nguyên tắc thiết kế cơ sở dữ liệu

Schema `retailbi_oltp` được tổ chức theo hướng chuẩn hóa **3NF (Third Normal Form)** để đảm bảo toàn vẹn dữ liệu trong vận hành. Tuy nhiên, nhóm có áp dụng kỹ thuật **denormalization có chủ đích** tại bảng `orders`. Bảng này lưu sẵn các trường tổng hợp như `total_amount`, `total_discount`, `net_amount` và `total_cost`, giúp việc export, đối soát và truy vấn tổng hợp từ tầng nguồn thuận tiện hơn mà không cần liên tục join lại với chi tiết đơn hàng.

![ERD OLTP - retailbi_oltp](erd_oltp.png)

*Hình 4.1. Entity Relationship Diagram (ERD) của schema retailbi_oltp*

#### Mô tả chi tiết 8 bảng thực thể

**Nhóm bảng danh mục (Reference/Master Data)**

| Bảng | Vai trò | Đặc điểm dữ liệu |
|---|---|---|
| `sales_channels` | Lưu 8 kênh bán hàng của VietTech (Physical, Web, App). | Kênh online (Web/App) cùng các cửa hàng vật lý tạo thành lớp phân tích hiệu quả theo kênh bán. |
| `product_categories` | Phân loại sản phẩm 2 cấp độ (category và sub-category). | 14 sub-categories bao phủ 6 nhóm hàng công nghệ chính. |
| `products` | Lưu trữ 47 mã SKU, giá vốn (`cost_price`) và giá bán niêm yết (`msrp`). | Giá tại bảng này phản ánh giá hiện tại ở cuối năm 2024, phục vụ lớp danh mục sản phẩm. |
| `customers` | Thông tin nhận dạng, nhân khẩu học và phân khúc kinh doanh (Consumer/Corporate/Home Office). | 5.000 hồ sơ khách hàng, là nguồn định danh để liên kết với đơn hàng trong các bước phân tích phía sau. |

**Nhóm bảng giao dịch (Transactional Data)**

| Bảng | Vai trò | Đặc điểm dữ liệu |
|---|---|---|
| `orders` | Bảng giao dịch trung tâm lưu header đơn hàng, trạng thái và tài chính tổng hợp. | 21.435 đơn hàng; có index trên `customer_id`, `status` và `order_date` để hỗ trợ truy vấn theo khách hàng, trạng thái đơn và thời gian đặt hàng. |
| `order_details` | Lưu từng dòng sản phẩm (SKU) trong đơn hàng, kèm giá bán, giá vốn và chiết khấu tại thời điểm mua. | Ràng buộc UNIQUE trên `order_id` và `product_id` không cho phép trùng sản phẩm trong cùng một đơn. |
| `order_returns` | Ghi nhận lịch sử hoàn trả đơn hàng, gồm lý do và thời điểm hoàn trả. | Quan hệ 1:1 với `orders`; tỷ lệ hoàn trả khoảng 6% tổng số đơn hàng. |
| `product_price_history` | Lưu vết các lần thay đổi giá MSRP và giá vốn theo thời gian. | Gồm `price_type`, `old_price`, `new_price` và `changed_at`, giúp truy vết lịch sử biến động giá sản phẩm. |
...
#### Quy trình sinh dữ liệu mô phỏng (Synthetic Data Generation)

Do không thể kết nối trực tiếp vào hệ thống OLTP production thực tế, dữ liệu trong schema `retailbi_oltp` được tạo ra thông qua script Python (`generate_data.py`) với thông số `SEED = 42` để đảm bảo tính tái lập (reproducibility). Dữ liệu được sinh ra mô phỏng hành vi mua sắm tại Việt Nam:

- **Nhóm hành vi nội bộ khi sinh dữ liệu:** Trong quá trình sinh dữ liệu, khách hàng được gán các nhãn hành vi nội bộ như VIP, Loyal, Regular, Occasional và Churned để điều chỉnh số lượng đơn hàng, thời điểm mua và mức độ hoạt động khác nhau. Các nhãn này chỉ phục vụ mô phỏng dữ liệu và không được ghi ra bảng `customers`.
- **Tính mùa vụ (Seasonality):** Phân phối đơn hàng tập trung cao vào các dịp cuối năm, tháng 12 và cận Tết Nguyên Đán (tháng 1, tháng 2), đồng thời giảm nhẹ vào quý 2.
- **Logic bán chéo (Cross-selling):** Hệ thống giả lập hành vi mua kèm phụ kiện, ví dụ khách mua smartphone có xác suất mua thêm ốp lưng, tai nghe hoặc cáp sạc.
- **Mô hình chiết khấu và thanh toán:** Phương thức thanh toán thay đổi theo kênh bán; Web/App ưu tiên E-Wallet, còn cửa hàng vật lý ưu tiên tiền mặt. Chiết khấu được áp dụng mạnh hơn vào các dịp lễ lớn.

Kết quả của tiến trình này là 8 file CSV snapshot trong thư mục `data/raw/`. Script cũng sinh file `data.sql` chứa các câu lệnh `INSERT` theo batch, giúp dựng lại source database trên MySQL khi cần.
...
Sau khi Source Database đã được dựng và vận hành với đầy đủ nghiệp vụ mô phỏng trong giai đoạn 2023–2024, dự án tiến hành xuất (export) toàn bộ dữ liệu ra dưới dạng các tệp văn bản thô. Dữ liệu này được đóng băng (snapshot) với thời điểm cắt dữ liệu (cut-off date) là **31/12/2024** và lưu trữ trong thư mục `data/raw/`.

Việc chuyển đổi từ SQL Database sang dạng file CSV nhằm thiết lập một ranh giới rõ ràng: **cô lập tiến trình phân tích BI (OLAP) khỏi hệ thống vận hành giao dịch (OLTP)**, đảm bảo không gây ảnh hưởng hiệu năng hệ thống nguồn.
...
Ở lớp này, dữ liệu hoàn toàn là nguyên bản (raw). Nhóm chưa áp dụng bất kỳ phép biến đổi (transform) hay làm sạch nào. Dữ liệu sẽ tiếp tục được đưa lên Cloud Data Warehouse (BigQuery) để bắt đầu quá trình mô hình hóa.
...
Việc tách lớp Staging có hai ý nghĩa quan trọng. Thứ nhất, đây là vùng đệm giúp kiểm tra row count và đối soát dữ liệu trước khi transform. Thứ hai, dữ liệu Staging vẫn giữ cấu trúc gần với nguồn CSV nên thuận tiện cho đối soát khi phát hiện sai lệch ở tầng Data Mart.
...
Trọng tâm của giai đoạn này là chuyển dữ liệu từ cấu trúc bảng nguồn sang mô hình Star Schema trong dataset `retailbi_mart`. Data Mart được chia thành hai nhóm bảng chính: dimension table để mô tả ngữ cảnh phân tích và fact table để lưu các sự kiện có thể đo lường được.

**Các bảng Dimension**

| Bảng | Vai trò | Cấu tạo chính |
|---|---|---|
| `dim_date` | Cung cấp trục thời gian dùng chung cho các phân tích theo ngày đặt hàng, ngày giao hàng và ngày hoàn trả. | Gồm `date_key`, ngày, năm, quý, tháng, tuần, thứ trong tuần, cờ cuối tuần, giai đoạn Tết và cuối năm. Bảng này cũng có các dòng đặc biệt `-1`, `-2`, `-3` để biểu diễn ngày không xác định, chưa giao và không áp dụng. |
| `dim_customer` | Mô tả khách hàng để lọc, nhóm và phân tích theo hồ sơ khách hàng. | Gồm `customer_key`, `customer_id`, mã khách hàng, họ tên, giới tính, thành phố, quốc gia, business segment và trạng thái hoạt động. |
| `dim_product` | Mô tả sản phẩm và danh mục sản phẩm phục vụ phân tích doanh thu, lợi nhuận theo sản phẩm. | Gồm `product_key`, `product_id`, mã/tên sản phẩm, category, sub-category, brand, giá niêm yết, giá vốn và trạng thái hoạt động. |
| `dim_channel` | Mô tả kênh bán hàng để phân tích hiệu quả theo kênh. | Gồm `channel_key`, `channel_id`, mã kênh, tên kênh, loại kênh, cờ online/offline và trạng thái hoạt động. |
| `dim_payment` | Mô tả phương thức thanh toán, giúp phân tích hành vi thanh toán và tránh lặp dữ liệu theo từng đơn hàng. | Gồm `payment_key`, phương thức thanh toán và nhóm thanh toán như Cash, Card hoặc Digital. Bảng này được tạo sau khi loại bỏ trùng lặp `payment_method`. |

**Các bảng Fact**

| Bảng | Vai trò | Cấu tạo chính |
|---|---|---|
| `fact_orders` | Lưu sự kiện ở cấp đơn hàng, là bảng fact chính để tính KPI tổng quan như số đơn, doanh thu, chiết khấu, net amount và cost. | Gồm `order_key`, `order_id`, mã đơn, `order_date_key`, `ship_date_key`, các khóa dimension khách hàng/kênh/thanh toán, trạng thái đơn và các chỉ số tiền tệ cấp đơn. |
| `fact_order_items` | Lưu sự kiện ở cấp dòng sản phẩm trong đơn hàng, phục vụ phân tích sản phẩm, số lượng bán, doanh thu dòng hàng và lợi nhuận dòng hàng. | Gồm `order_item_key`, `order_key`, `product_key`, `order_date_key`, số lượng, đơn giá, giá vốn, tỷ lệ/giá trị chiết khấu, `line_total`, `line_cost` và `line_profit`. |
| `fact_returns` | Lưu sự kiện hoàn trả, giúp phân tích đơn hàng bị trả lại và lý do hoàn trả. | Gồm `return_key`, `order_key`, `returned_date_key` và `reason`. Bảng vẫn giữ `order_id` để đối soát với dữ liệu nguồn khi cần. |

Trong mô hình này, các dimension đóng vai trò là lớp ngữ cảnh để lọc và nhóm dữ liệu, còn các fact là lớp sự kiện dùng để tính toán chỉ số. `fact_orders` liên kết với `dim_customer`, `dim_channel`, `dim_payment` và `dim_date`. `fact_order_items` liên kết với `fact_orders`, `dim_product` và `dim_date`. `fact_returns` liên kết với `fact_orders` và `dim_date`. Các fact table không còn join trực tiếp bằng khóa tự nhiên của dữ liệu nguồn mà dùng surrogate key để kết nối với dimension. Riêng `fact_order_items` và `fact_returns` vẫn giữ `order_id` cho mục đích đối soát, nhưng đồng thời bổ sung `order_key` để liên kết nhất quán với `fact_orders` ở tầng BI.
...
**Chuẩn hóa định dạng dữ liệu khi load Data Mart:** Dữ liệu ở Staging được giữ gần với nguồn CSV nên có thể chưa phù hợp trực tiếp cho phân tích. Khi load sang Data Mart, nhóm ép kiểu các trường định danh về `INT64`, các trường tiền tệ về `NUMERIC`, và chuẩn hóa các trường boolean hoặc text để dữ liệu nhất quán hơn trong mô hình phân tích.
...
**Phân tách đa ngữ nghĩa của trường `ship_date` bị khuyết thiếu:** Trong dữ liệu nguồn, trường `ship_date` xuất hiện nhiều giá trị `NULL`, tuy nhiên các giá trị này mang các bản chất nghiệp vụ hoàn toàn khác nhau. Để tránh hiện tượng nhập nhằng ngữ nghĩa (Semantic Overloading), dự án không gộp chung các giá trị `NULL` này mà thực hiện phân tách thành ba mã định danh cụ thể tại tầng dữ liệu: `-1` áp dụng cho dữ liệu bất thường, lỗi hệ thống hoặc không xác định (Unknown/Missing); `-2` áp dụng cho các đơn hàng đang ở trạng thái chờ xử lý (`Pending`/`Processing`) và chưa đến ngày giao; `-3` áp dụng cho các đơn hàng đã bị hủy (`Cancelled`), hoàn toàn không phát sinh ngày giao. Giải pháp xử lý này giúp các báo cáo vận hành không hiểu sai cùng một giá trị `NULL` theo nhiều nghĩa khác nhau.
...
Ba điểm số này sau đó được ghép lại thành `rfm_score`, ví dụ `555` thể hiện nhóm khách hàng có điểm cao ở cả ba tiêu chí. Dựa trên tổ hợp điểm R, F, M, hệ thống gán khách hàng vào các nhóm hành vi như `Champions`, `Loyal`, `At-risk`, `Churned` và `Regular`. Việc phân nhóm được thực hiện sẵn ở tầng Data Mart thay vì tính động trong Power BI để đảm bảo kết quả ổn định và nhất quán khi trình bày Dashboard.
...
- **Thống nhất một phần logic KPI và RFM:** Các định nghĩa quan trọng như completed revenue, completed orders và phân khúc RFM được tổ chức ở tầng Data Mart/BI Views, giúp giảm rủi ro mỗi Dashboard diễn giải chỉ số theo một cách khác nhau.
- **Giảm bớt xử lý ở tầng trực quan hóa:** Power BI sử dụng dữ liệu đã được mô hình hóa sẵn từ Data Mart, nên tập trung nhiều hơn vào trình bày, lọc và so sánh dữ liệu thay vì phải tái tạo toàn bộ logic transform từ dữ liệu thô.
...
| RFM Dashboard | Khách hàng đang được phân nhóm ra sao theo hành vi mua hàng? | Số khách theo `rfm_segment`, giá trị chi tiêu theo từng phân khúc RFM, bảng chi tiết khách hàng, độ gần đây, tần suất mua, gợi ý hành động theo từng phân khúc RFM | `mart_rfm_snapshot`, `dim_customer` |
...
- Treemap thể hiện số khách trong RFM Snapshot theo `rfm_segment`.
- Biểu đồ cột thể hiện giá trị chi tiêu theo từng phân khúc RFM.
- Bảng chi tiết khách hàng gồm thành phố, `rfm_segment`, `monetary_value`, `recency_days`, `frequency_orders` và gợi ý hành động theo từng phân khúc RFM.
...
Những kết quả mang giá trị thực tiễn và học thuật cốt lõi mà dự án đã đạt được bao gồm:

- **Tối ưu hóa hạ tầng dữ liệu:** Chuyển đổi thành công nguồn dữ liệu thô thành các mô hình phân tích có cấu trúc, gia tăng hiệu năng truy vấn hệ thống.
- **Đồng bộ hóa logic nghiệp vụ:** Loại bỏ hoàn toàn sự bất đồng bộ trong cách diễn giải chỉ số KPI nhờ việc nhất quán logic từ tầng Data Mart, BI Views lên tới báo cáo cuối cùng.
- **Khai thác sâu hành vi khách hàng:** Ứng dụng thành công mô hình RFM vào kho dữ liệu, giúp phân tách rõ ràng các phân khúc khách hàng chiến lược.
- **Nâng cao năng lực trực quan hóa:** Hoàn thiện giải pháp Dashboard quản trị đa chiều trên Power BI dựa trên nguồn dữ liệu trung gian đã được làm sạch và chuẩn hóa.

---

## PHỤ LỤC A: MÃ NGUỒN VÀ TÀI LIỆU DỰ ÁN

Toàn bộ mã nguồn, tài liệu thiết kế và dữ liệu liên quan của dự án được lưu trữ trực tuyến tại hệ thống GitHub. Người đọc có thể truy cập và tham khảo chi tiết tại đường dẫn:

Địa chỉ repository: [**https://github.com/SkyAnzu/viettech-bi-project**](https://github.com/SkyAnzu/viettech-bi-project)
